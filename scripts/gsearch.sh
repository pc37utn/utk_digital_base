...
# Deploy GSearch
cp -v /tmp/gsearch/FgsBuild/fromsource/fedoragsearch.war /var/lib/tomcat/webapps
...
sed -i 's#JAVA_OPTS="-Djava.awt.headless=true -Xmx128m -XX:+UseConcMarkSweepGC"#JAVA_OPTS="-Djava.awt.headless=true -Xmx1024m -XX:MaxPermSize=256m -XX:+UseConcMarkSweepGC -Dkakadu.home=/usr/local/djatoka/bin/Linux-x86-64 -Djava.library.path=/usr/local/djatoka/lib/Linux-x86-64 -DLD_LIBRARY_PATH=/usr/local/djatoka/lib/Linux-x86-64"#g' /etc/profile.d/islandora.sh
systemctl restart tomcat
sleep 30

# GSearch configurations
cd /var/lib/tomcat/webapps/fedoragsearch/WEB-INF/classes
wget -q http://alpha.library.yorku.ca/fgsconfigFinal.zip
unzip fgsconfigFinal.zip

# Deploy dgi_gsearch_extensions
cp -v /tmp/dgi_gsearch_extensions/target/gsearch_extensions-0.1.1-jar-with-dependencies.jar /var/lib/tomcat/webapps/fedoragsearch/WEB-INF/lib

# Solr & GSearch configurations
cp -v /tmp/basic-solr-config/conf/* /usr/local/solr/collection1/conf
cp -Rv /tmp/basic-solr-config/islandora_transforms/* /var/lib/tomcat/webapps/fedoragsearch/WEB-INF/classes/fgsconfigFinal/index/FgsIndex/islandora_transforms
chown -hR tomcat:tomcat /usr/local/solr
chown -hR tomcat:tomcat /var/lib/tomcat/webapps/fedoragsearch
